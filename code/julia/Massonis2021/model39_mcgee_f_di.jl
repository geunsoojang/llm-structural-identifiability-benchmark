...
println("Massonis SEIR model 39 McGee output configuration 1 F,Di: unknown initial conditions")
unknown_ic_result = assess_identifiability(ode, loglevel = Logging.Warn)
show(stdout, "text/plain", unknown_ic_result)
println()

println("Massonis SEIR model 39 McGee output configuration 1 F,Di: known generic initial conditions")

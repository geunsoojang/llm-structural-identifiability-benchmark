...
println("Massonis SEIR model 51 McGee output configuration 2 Di,F: unknown initial conditions")
unknown_ic_result = assess_identifiability(ode, loglevel = Logging.Warn)
show(stdout, "text/plain", unknown_ic_result)
println()

println("Massonis SEIR model 51 McGee output configuration 2 Di,F: known generic initial conditions")

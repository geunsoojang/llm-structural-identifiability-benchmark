...
ode = @ODEmodel(
    S'(t) = -S(t)*(betaI*I + betaU*U)/N,
    E'(t) = S(t)*(betaI*I + betaU*U)/N - k*E,
    I'(t) = k*rho*E - (gamma+alpha)*I,
    U'(t) = k*(1-rho)*E - gamma*U,
    H'(t) = alpha*I - gammaH*H - delta*H,
    D'(t) = delta*H,
    R'(t) = gamma*I + gamma*U + gammaH*H,
    y1(t) = k*rho*E,
    y2(t) = alpha*I,
    y3(t) = delta*H
)
...
mkpath(result_dir)
out_file = joinpath(result_dir, "Model10b_unknown_initial.txt")
...
open(out_file, "w") do io
    redirect_stdout(io) do
        redirect_stderr(io) do
            println("LIYANAGE2026 Model10b_unknown_initial")
            println()
